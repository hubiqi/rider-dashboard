...
(function(){
  ...
  async function parseXlsx(file, onProgress){
    ...
    return {
      ...
      fmt: s.fmt,
      meta: { total: s.rows, to: to, comp: Math.round(comp), src: file.name }
    };
  }
  /* ================= 概览 ================= */
  function renderKPI(){
  ...
  $('#fileInput').addEventListener('change', async function(e){
    ...
    try{
      ...
      setStatus('✅ 已载入 <b>'+esc(f.name)+'</b>：'+dataSummary());
    } catch(err){
      setStatus('❌ 解析失败：'+esc(err.message), true);
    }
    e.target.value = '';
  });
  ...
  /* ================= 初始化 ================= */
  window.__build = 'v22';   // 版本标记（便于排查缓存）
  function renderAll(){
    renderFilterBar();
    LASTBYD = renderKPI();
    drawTrend(LASTBYD);
  ...
  $('#method').innerHTML =
    ...
    '① <b>新格式</b>（运单明细）：需含列 骑手id、平台期望时间、超平台期望送达时长；'+
    '「是否二呼单」直接取值，「异常报备项」含「商户出货慢」即判为出餐慢；'+
    '② <b>旧格式</b>：需含列 日期、骑手id、是否准时单（考核）、复合超时时长（秒）、是否出餐慢报备。'+
